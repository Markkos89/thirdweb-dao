...
const App = () => {
  ...
  const { contract: token } = useContract("<CONTRACT_ADDRESS>", "token");
  const { contract: vote } = useContract("<CONTRACT_ADDRESS>", "vote");
  ...
  useEffect(() => {
    ...
    const getAllProposals = async () => {
      if (vote) {
        try {
          const proposals: Proposal[] = await vote.getAll();
  ...
  useEffect(() => {
    ...
    const getAllBalances = async () => {
      if (token) {
        try {
          const amounts: TokenHolderBalance[] =
